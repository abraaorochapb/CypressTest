...
describe('Newsletter subscription form Tests', () => {
  ...
  it('Should allow users to subscribe with valid email', () => {
    cy.getByData("email-input").type("zezinho@teste").then(() => {
      cy.getByData("submit-button").click()
    })
    
    cy.getByData("success-message").invoke('text').should('include', 'Success').and('include', 'zezinho@teste')
  });

  it('Should NOT allow users to subscribe with invalid email', () => {
    cy.getByData("email-input").type("zezinho").then(() => {
      cy.getByData("submit-button").click()
    })
  ...
  it.only('Should NOT allow users to subscribe twice with the same email', () => {
    cy.getByData("email-input").type("john@example.com").then(() => {
      cy.getByData("submit-button").click()
    })
